...
def test_get_schedule():
    response = client.get("/v1/api/get_schedule")
    assert response.status_code == 200
...
def test_get_scheduler_log():
    response = client.get("/v1/api/scheduler_log")
    assert response.status_code == 200
    logs = response.json()
    assert isinstance(logs, list)
    assert len(logs) > 0
...
def test_get_serial_log():
    response = client.get("/v1/api/serial_log")
    assert response.status_code == 200
    logs = response.json()
    assert isinstance(logs, list)
    assert len(logs) > 0